"""CUDA-first, bounded renderer-corrected TileNet v2 trainer."""
...
# v2 owns its locks and run boundary, while the model and validated vector loader
# remain the reviewed v1 implementations.
...
def _canonical_manifest_sha256(data_dir: Path) -> str:
    ...
    canonical = Path(__file__).resolve().parent / "manifests" / "dataset-v2.json"
    try:
    ...
    if actual_hash != canonical_hash:
        raise TrainingError("generated dataset manifest does not match the committed dataset-v2 manifest")
...
def _verify_data_quality(
    ...
    """Require pre-training renderer and visual-review evidence bound to this v2 corpus."""
    experiment_root = experiment_root or Path(__file__).resolve().parent
    quality_path = quality_path or experiment_root / "manifests" / "data-quality.json"
    try:
        quality = json.loads(quality_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise TrainingError("v2 data-quality evidence is unavailable") from error
    if not isinstance(quality, dict) or quality.get("schemaVersion") != 1:
        raise TrainingError("v2 data-quality evidence schema is invalid")
    if quality.get("status") != "passed":
        raise TrainingError("v2 data-quality evidence did not pass")
    if quality.get("datasetManifestSha256") != dataset_manifest_hash:
        raise TrainingError("v2 data-quality evidence does not bind this dataset")
    if quality.get("protocolSha256") != protocol_hash:
        raise TrainingError("v2 data-quality evidence does not bind this protocol")
    svg_digest = quality.get("svgFidelitySha256")
    if not isinstance(svg_digest, str) or not re.fullmatch(r"[a-f0-9]{64}", svg_digest):
        raise TrainingError("v2 data-quality SVG fidelity identity is invalid")
    fidelity_path = fidelity_path or experiment_root / "reports" / "svg-fidelity.json"
    try:
        fidelity = json.loads(fidelity_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise TrainingError("v2 SVG fidelity evidence is unavailable") from error
    if sha256_file(fidelity_path) != svg_digest or not isinstance(fidelity, dict) or fidelity.get("status") != "passed":
        raise TrainingError("v2 SVG fidelity evidence is not the reviewed passing artifact")
    automated_digest = quality.get("automatedQualitySha256")
    automated_path = experiment_root / "reports" / "automated-quality.json"
    try:
        automated = json.loads(automated_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise TrainingError("v2 automated quality evidence is unavailable") from error
    if (
        not isinstance(automated_digest, str)
        or not re.fullmatch(r"[a-f0-9]{64}", automated_digest)
        or sha256_file(automated_path) != automated_digest
        or not isinstance(automated, dict)
        or automated.get("status") != "passed"
        or automated.get("datasetManifestSha256") != dataset_manifest_hash
        or automated.get("protocolSha256") != protocol_hash
    ):
        raise TrainingError("v2 automated quality evidence is not the reviewed passing artifact")
    checks = quality.get("checks")
    if not isinstance(checks, dict) or not checks or not all(value is True for value in checks.values()):
        raise TrainingError("v2 data-quality checks are incomplete or failed")
    visual_review = quality.get("visualReview")
    if not isinstance(visual_review, dict) or visual_review.get("status") != "passed":
        raise TrainingError("v2 visual review did not pass")
    reviewer = visual_review.get("reviewer")
    artifacts = visual_review.get("artifactSha256")
    if not isinstance(reviewer, str) or not reviewer or not isinstance(artifacts, dict) or not artifacts:
        raise TrainingError("v2 visual review identity is incomplete")
    if not all(isinstance(path, str) and path and isinstance(digest, str) and re.fullmatch(r"[a-f0-9]{64}", digest) for path, digest in artifacts.items()):
        raise TrainingError("v2 visual review artifact identities are invalid")
    ...
    for relative_path, digest in artifacts.items():
        ...
        try:
            ...
            raise TrainingError("v2 visual review artifact escapes the experiment") from error
        try:
            if sha256_file(artifact_path) != digest:
                raise TrainingError("v2 visual review artifact differs from the reviewed evidence")
        except OSError as error:
            raise TrainingError("v2 visual review artifact is unavailable") from error
    return sha256_file(quality_path)
...
def _implementation_hashes() -> dict[str, str]:
    ...
    try:
        return {
            ...
            "shared/dataset.py": sha256_file(LEGACY_ROOT / "dataset.py"),
            "shared/tilenet_model.py": sha256_file(LEGACY_ROOT / "tilenet_model.py"),
            "shared/requirements.lock": sha256_file(LEGACY_ROOT / "requirements.lock"),
        }
    except OSError as error:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        data_quality_sha256 = _verify_data_quality(args.data_dir, protocol_hash, manifest_sha256)
        if recipe.mode == "full":
        ...
        base_report.update(
            {
                ...
                "dataQualitySha256": data_quality_sha256,
