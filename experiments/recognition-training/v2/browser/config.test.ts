...
function fixture() {
  ...
  writeJson(freezePath, {
    ...
    candidates: [
      ...
      candidate('tilenet-3811', 3811),
      candidate('tilenet-3812', 3812),
    ],
  });
  ...
  return { configPath, vectorManifestPath, freezePath };
}
...
describe('browser evaluation frozen configuration', () => {
  it('accepts the exact full-run seeds and locked held-out manifest', () => {
    ...
    expect(config.freeze.candidates.map(({ seed }) => seed)).toEqual([null, 3811, 3812]);
...
it('rejects legacy full seeds instead of silently accepting a different experiment', () => {
  const input = fixture();
  const value: unknown = JSON.parse(readFileSync(input.freezePath, 'utf8'));
  if (
    typeof value !== 'object' ||
    value === null ||
    !('candidates' in value) ||
  ...
  for (const candidate of value.candidates as { seed: number | null }[]) {
    if (candidate.seed === 3811) candidate.seed = 3801;
    if (candidate.seed === 3812) candidate.seed = 3802;
  }
  writeJson(input.freezePath, value);
  expect(() => loadBrowserEvaluationConfig(input.configPath)).toThrow('3811');
});
