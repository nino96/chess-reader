...
import { integer, makeRandom, randomPosition, sampleSeed } from './recipe.mjs';
import {
  EXPERIMENT_ROOT,
  assertLittleEndian,
  assertNoCorpusV1,
...
function pieceCode(classIndex) {
  const piece = CLASS_ORDER[classIndex];
  if (piece === undefined || piece === '1') return null;
...
async function loadFamilyImages(family) {
  ...
  for (const code of PIECE_CODES) images.set(code, await loadImage(resolve(root, `${code}.svg`)));
  return images;
}
...
/** @param {readonly number[]} labels @param {Map<string, import('@napi-rs/canvas').Image>} pieces @param {number} seed @param {boolean} includePreview */
function renderTiles(labels, pieces, seed, includePreview) {
  ...
  const style = styleForBoard(random);
  ...
  const reduction = [1, 0.82, 0.64][integer(0, 2, random)];
  ...
  const speckles = Math.floor(boardPixels * boardPixels * (random() < 0.5 ? 0 : 0.0015));
...
async function generateSplit(split, count) {
  ...
  /** @type {{ id: string; family: string; labels: number[] }[]} */
  const boards = [];
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    boards.push({
      id: `synthetic-${split}-${family}-${String(index).padStart(5, '0')}`,
      family,
      labels,
    });
...
const generatorDependencies = {
  ...
  'pnpm-lock.yaml': await generatorHash(resolve(EXPERIMENT_ROOT, '..', '..', 'pnpm-lock.yaml')),
};
const manifest = {
  schemaVersion: 1,
  id: 'synthetic-tilenet-v1',
  seed: DATASET_SEED,
